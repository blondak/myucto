...
final class AccountingArchiveCatalog
{
    ...
    public function forExport(): array
    {
        ...
        usort(
            $tables,
            static fn (AccountingArchiveTable $left, AccountingArchiveTable $right): int =>
                $left->exportOrder <=> $right->exportOrder,
        );
        return $tables;
    }
    ...
    private function tables(): array
    {
        ...
        if (!$this->registry->isComplete(TenantDataRegistry::ACCOUNTING_ARCHIVE_PROFILE)) {
            throw new \LogicException('Účetní archivní profil tenantového registru není úplný.');
        }

        $tables = [];
        $exportOrders = [];
        $restoreOrders = [];
        foreach ($this->registry->definitionsFor(TenantDataRegistry::ACCOUNTING_ARCHIVE_PROFILE) as $definition) {
        ...
        if ($tables === []) {
            throw new \LogicException('Účetní archivní profil neobsahuje žádné tabulky.');
        }
