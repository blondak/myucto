...
final readonly class AccountingArchiveTable
{
    ...
    public static function fromDefinition(TenantDataDefinition $definition): self
    {
        if ($definition->kind !== TenantDataObjectKind::Table
            || !str_starts_with($definition->key, TenantDataObjectKind::Table->keyPrefix())
        ) {
            throw new \LogicException('Účetní archiv obsahuje objekt, který není tabulkou.');
        }
        ...
        if ($primaryKey === []) {
            throw new \LogicException('Tabulka účetního archivu musí mít stabilní primární klíč.');
        }

        $archive = $definition->details['accounting_archive'] ?? null;
        if (!is_array($archive) || array_is_list($archive)) {
            throw new \LogicException('Tabulka účetního archivu nemá projekční metadata.');
        }
        $exportOrder = $archive['export_order'] ?? null;
        $restoreOrder = $archive['restore_order'] ?? null;
        if (!is_int($exportOrder) || $exportOrder < 1
            || !is_int($restoreOrder) || $restoreOrder < 1
        ) {
            throw new \LogicException('Tabulka účetního archivu nemá platné stabilní pořadí.');
        }
        $selector = $archive['selector'] ?? null;
        if (!is_string($selector) || !in_array($selector, [
            'ownership',
            'bank_transaction_relationships',
            'bank_statement_relationships',
            'accounting_period_currency',
        ], true)) {
            throw new \LogicException('Tabulka účetního archivu má neznámý selektor.');
        }

        $ownership = $definition->details['ownership'] ?? null;
        if (!is_array($ownership) || array_is_list($ownership)) {
            throw new \LogicException('Tabulka účetního archivu má neplatné vlastnictví.');
        }
        $omitColumns = self::identifierList(
            $archive['omit_columns'] ?? null,
            'vynechané sloupce tabulky ' . $name,
        );
        $secretPolicies = self::secretPolicies($definition->details['secrets'] ?? null, $name);
        ...
        sort($omitColumns, SORT_STRING);

        $softReferences = self::referenceMap($archive['soft_references'] ?? null, $name);
        $featureFlag = $archive['feature_flag'] ?? null;
        if ($featureFlag !== null && $featureFlag !== 'stock_enabled') {
            throw new \LogicException('Tabulka účetního archivu má neznámý feature flag.');
        }
    ...
    private static function identifierList(mixed $value, string $label): array
    {
        if (!is_array($value) || !array_is_list($value)) {
            throw new \LogicException('Účetní archiv má neplatnou deklaraci: ' . $label . '.');
        }
        $result = [];
        foreach ($value as $identifier) {
            ...
            if (in_array($identifier, $result, true)) {
                throw new \LogicException('Účetní archiv má duplicitní deklaraci: ' . $label . '.');
            }
            $result[] = $identifier;
        }
        return $result;
    ...
    private static function referenceMap(mixed $value, string $table): array
    {
        if (!is_array($value) || array_is_list($value) && $value !== []) {
            throw new \LogicException('Tabulka účetního archivu má neplatné soft reference.');
        }
        $result = [];
    ...
    {
        if (!is_array($value) || array_is_list($value) && $value !== []) {
            throw new \LogicException('Tabulka účetního archivu má neplatný registr secrets.');
        }
        $result = [];
        foreach ($value as $column => $declaration) {
            if (!is_string($column) || !is_array($declaration) || array_is_list($declaration)) {
                throw new \LogicException('Tabulka účetního archivu má neplatný registr secrets.');
            }
            self::assertIdentifier($column, 'secret sloupec tabulky ' . $table);
            $policyValue = $declaration['policy'] ?? null;
            $policy = is_string($policyValue) ? TenantSecretPolicy::tryFrom($policyValue) : null;
            if ($policy === null) {
                throw new \LogicException('Tabulka účetního archivu má neznámou secret politiku.');
            }
    ...
    private static function assertIdentifier(string $value, string $label): void
    {
        if (preg_match('/^[a-z][a-z0-9_]{0,63}$/D', $value) !== 1) {
            throw new \LogicException('Účetní archiv má neplatný ' . $label . '.');
        }
    }
}
