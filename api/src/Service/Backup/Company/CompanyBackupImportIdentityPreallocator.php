...
/**
 * Před zápisem rezervuje cílový primární klíč a všechny jeho obnovovací
 * aliasy. Payload ani reference, které identitu netvoří, přitom nemění.
 */
final class CompanyBackupImportIdentityPreallocator
{
    ...
    public function __construct(
        ...
        if (!in_array($definition->policy, [
            ...
            throw self::error(
                'import_identity_preallocation_context_mismatch',
                $definition->key,
            );
        }
    ...
    public function preallocate(
        array $sourceRow,
    ): CompanyBackupPreallocatedImportIdentity {
        if ($this->closed) {
            throw self::error(
                'import_identity_preallocator_closed',
                $this->projection->registryKey,
            );
        }

        $this->projection->assertCompleteSourceRow($sourceRow);
        $sourceIdentity = $this->identityProjection->identityForRow($sourceRow);
        $targetSeed = $sourceRow;
        if ($this->primaryKeys !== null) {
            $targetSeed[$this->projection->primaryKey[0]] =
                $this->primaryKeys->next();
        }
        $targetIdentityRow = $this->transformer->transformIdentityColumnReferences(
            $this->projection,
            $targetSeed,
            $this->identityProjection->identityColumns(),
        );
        $targetIdentityRow = $this->projection->restoreOverrides->apply(
            $targetIdentityRow,
        );
        $targetIdentity = $this->identityProjection->identityForRow(
            $targetIdentityRow,
        );
        $this->identities->add($sourceIdentity, $targetIdentity);

        return new CompanyBackupPreallocatedImportIdentity(
            $sourceIdentity,
            $targetIdentity,
        );
    }
