    ...
    private function createInvoiceFromTemplate(array $template, string $issueDate, ?int $userId): int
    {
        ...
        try {
            ...
            $stmt->execute([
                ...
                (int) $template['id'],
                $revenueCategoryId,
                // Stabilní platební VS šablony (#249) — trvalý příkaz zákazníka.
                InvoiceRepository::normalizePaymentVariableSymbol($template['payment_variable_symbol'] ?? null),
                $userId,
            ]);
            ...
            foreach ($template['items'] as $item) {
                ...
                $items[] = [
                    ...
                    // Ručně zvolená klasifikace DPH (migrace 1783). Bez ní derivace
