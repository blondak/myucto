 ...
 *     559 účetní OP) na straně MD uvnitř období, po zohlednění skutečných storen
 *     těchto zápisů. Čerpání a rozpuštění rezerv se do tvorby nezapočítává.
...
final class LegalProvisionLedgerService
{
    ...
    /**
     * Zůstatek tvoří počáteční stav a pohyby daného období. Historické obraty se
     * nepřičítají znovu k otevření knih. ClosingRepository::accountBalance naopak
     * sčítá historii včetně minulých uzavíracích zápisů, které zde vylučujeme.
     *
    ...
    private function creditBalance(int $supplierId, int $periodId, string $accountCode, string $asOf): float
    {
        ...
        $stmt->execute([$supplierId, $periodId, $asOf, ClosingSourceId::STOCK_SLOT_BASE, $accountCode, $accountCode]);

        return round((float) $stmt->fetchColumn(), 2);
    }

    /**
     * Tvorba zahrnuje MD zápisy a jejich skutečné storno řetězce. Běžný zápis
     * čerpání na D ji nesnižuje. Odpis pohledávky zachovává čistý daňový náklad.
     *
