...
final class OpenItemsPairingTest extends TestCase
{
    ...
    public function testRedStornoPairsWithOriginalOnSamePhysicalSide(): void
    {
        $transit = $this->accountId('261');
        $original = $this->manual([self::l('261', 'debit', 100.00), self::l('221', 'credit', 100.00)], '-03-10');
        $red = $this->manual([
            self::l('261', 'debit', 30.00) + ['is_red_storno' => true],
            self::l('221', 'credit', 30.00) + ['is_red_storno' => true],
        ], '-03-11');

        $pairing = $this->openItems->create($this->supplierId, $transit,
            [$this->lineId($original, '261'), $this->lineId($red, '261')], null, $this->meta);
        self::assertSame(70.0, $pairing['remainder']);

        $open = $this->openItems->build($this->supplierId, $transit, self::YEAR . '-12-31', true, 1, 100);
        self::assertSame(70.0, $open['balance']);
        self::assertSame(70.0, $open['open_total']);
        self::assertSame(0.0, $open['difference']);
        self::assertSame(1, $open['open_count']);
        self::assertSame($original, $open['items'][0]['entry_id']);
        self::assertSame(70.0, $open['items'][0]['open_amount']);
        $all = $this->openItems->build($this->supplierId, $transit, self::YEAR . '-12-31', false, 1, 100);
        $redRow = array_values(array_filter($all['items'], static fn (array $row): bool => $row['entry_id'] === $red))[0];
        self::assertSame('debit', $redRow['side']);
        self::assertTrue($redRow['is_red_storno']);
        self::assertSame(30.0, $redRow['amount']);
    }
