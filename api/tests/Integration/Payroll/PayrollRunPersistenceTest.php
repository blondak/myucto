...
final class PayrollRunPersistenceTest extends TestCase
{
    ...
    public function testCompanyBackupStreamsPersonAddressHistory(): void
    {
        $insert = $this->db->pdo()->prepare(
            'INSERT INTO payroll_person_addresses
                (supplier_id, employee_id, address_type, street_line, city,
                 postal_code, country_code, effective_from, effective_to,
        ...
        $residenceId = (int) $this->db->pdo()->lastInsertId();
        $insert->execute([
            $this->supplierId,
            $this->employeeId,
            'mailing',
        ...
        $mailingId = (int) $this->db->pdo()->lastInsertId();

        $this->db->pdo()->prepare(
            'INSERT INTO payroll_employees
                (supplier_id, full_name, taxpayer_type, is_active)
             VALUES (?, "Synthetic Foreign Address Person", "employee", 1)'
        )->execute([$this->otherSupplierId]);
        $otherEmployeeId = (int) $this->db->pdo()->lastInsertId();
        $insert->execute([
            $this->otherSupplierId,
        ...
        $residence = $rows[0];
        $mailing = $rows[1];
        self::assertSame($residenceId, (int) $residence['id']);
        self::assertSame($this->supplierId, (int) $residence['supplier_id']);
        self::assertSame($this->employeeId, (int) $residence['employee_id']);
        self::assertSame('residence', $residence['address_type']);
        self::assertSame('Syntetická 12', $residence['street_line']);
        self::assertSame('Testov', $residence['city']);
        self::assertSame('123 45', $residence['postal_code']);
        self::assertSame('CZ', $residence['country_code']);
        self::assertSame('2026-01-01', $residence['effective_from']);
        self::assertNull($residence['effective_to']);
        self::assertSame(5, (int) $residence['row_version']);
        self::assertSame('2026-01-02 10:00:00', $residence['created_at']);
        self::assertSame('2026-06-01 11:00:00', $residence['updated_at']);

        self::assertSame($mailingId, (int) $mailing['id']);
        self::assertSame('mailing', $mailing['address_type']);
        self::assertSame('Modelová 34', $mailing['street_line']);
        self::assertSame('Ukázkov', $mailing['city']);
        self::assertSame('987 65', $mailing['postal_code']);
        self::assertSame('SK', $mailing['country_code']);
        self::assertSame('2026-02-01', $mailing['effective_from']);
        self::assertSame('2026-05-31', $mailing['effective_to']);
        self::assertSame(2, (int) $mailing['row_version']);
        self::assertSame('2026-02-02 12:00:00', $mailing['created_at']);
        self::assertSame('2026-05-31 17:00:00', $mailing['updated_at']);
    }
