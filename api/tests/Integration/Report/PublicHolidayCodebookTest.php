...
/**
 * P-7 — celý řetěz od tabulky `public_holidays` (migrace 1781) až k lhůtě podání.
 *
...
final class PublicHolidayCodebookTest extends TestCase
{
    public function testImpossibleFixedDatesAreRejectedBeforePersistence(): void
    {
        ...
        foreach (['02-30', '02-31', '04-31', '06-31', '09-31', '11-31'] as $monthDay) {
            $request = (new \Slim\Psr7\Factory\ServerRequestFactory())->createServerRequest('POST', '/api/codebooks/public-holidays')
                ...
                ->withParsedBody(['code' => 'fixture_invalid_date', 'name' => 'Fixture', 'rule_type' => 'fixed', 'month_day' => $monthDay, 'valid_from' => '2099-01-01']);
            $response = $action->create($request, (new \Slim\Psr7\Factory\ResponseFactory())->createResponse());
            self::assertSame(422, $response->getStatusCode(), $monthDay);
        }
    }
    ...
    protected function setUp(): void
    {
        ...
        if ($seeded === 0) {
            $this->markTestSkipped('Číselník svátků není naseedovaný (migrace 1781).');
        }
    ...
    /**
     * Seed migrace 1781 dává TÝŽ výsledek jako dosavadní konstanta: 25. 7. 2026
