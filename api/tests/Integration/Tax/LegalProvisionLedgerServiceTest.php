...
final class LegalProvisionLedgerServiceTest extends TestCase
{
    ...
    /** Tvorba rezervy a její čerpání jsou oddělené údaje. */
    public function testLegalRepairReserveBalanceAndCreation(): void
    {
        ...
        self::assertEqualsWithDelta(100000.00, $data['legal_reserve_created'], 0.001);
        self::assertTrue($data['has_activity']);
    }
    ...
    public function testOpeningBalanceDoesNotCountPriorPeriodCreationAgain(): void
    {
        $year = self::YEAR - 1;
        $this->periods->create($this->supplierId, $year, $year . '-01-01', $year . '-12-31');
        $this->manualEntry($year . '-06-30', '552', '451', 100000.00);
        $this->posting->postDocument($this->supplierId, 'opening', $this->periodId, [
            ['account_code' => '701', 'side' => 'debit', 'amount' => 100000.00],
            ['account_code' => '451', 'side' => 'credit', 'amount' => 100000.00],
        ], ['entry_date' => self::STARTS_ON, 'posted' => true, 'posted_by' => $this->userId, 'user_id' => $this->userId]);
        self::assertSame(100000.0, $this->forPeriod()['legal_reserve_balance']);
        self::assertSame(0.0, $this->forPeriod()['legal_reserve_created']);
    }

    public function testActualReversalsAreDistinguishedFromReserveRelease(): void
    {
        $creation = $this->manualEntry(self::YEAR . '-06-30', '552', '451', 100000.00);
        $release = $this->manualEntry(self::YEAR . '-07-30', '451', '552', 30000.00);
        $reversal = $this->posting->reverse($this->supplierId, $creation, ['entry_date' => self::YEAR . '-08-01', 'posted_by' => $this->userId, 'user_id' => $this->userId]);
        self::assertSame(0.0, $this->forPeriod()['legal_reserve_created']);
        $this->posting->reverse($this->supplierId, $release, ['entry_date' => self::YEAR . '-08-02', 'posted_by' => $this->userId, 'user_id' => $this->userId]);
        self::assertSame(0.0, $this->forPeriod()['legal_reserve_created']);
        $this->posting->reverse($this->supplierId, $reversal, ['entry_date' => self::YEAR . '-08-03', 'posted_by' => $this->userId, 'user_id' => $this->userId]);
        self::assertSame(100000.0, $this->forPeriod()['legal_reserve_created']);
    }
    ...
    private function manualEntry(string $date, string $debit, string $credit, float $amount): int
    {
