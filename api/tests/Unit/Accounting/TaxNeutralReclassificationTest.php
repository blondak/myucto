...
final class TaxNeutralReclassificationTest extends TestCase
{
    ...
    public function testRedDebitAndOrdinaryCreditHaveSameNetMovement(): void
    {
        $red = [
            ['account_id' => 1, 'side' => 'debit', 'amount' => 100.00, 'is_red_storno' => true],
            ['account_id' => 5, 'side' => 'credit', 'amount' => 100.00, 'is_red_storno' => true],
        ];
        $ordinary = [
            ['account_id' => 1, 'side' => 'credit', 'amount' => 100.00],
            ['account_id' => 5, 'side' => 'debit', 'amount' => 100.00],
        ];

        self::assertNull(R::violation($red, $ordinary, self::ACCOUNTS));
        self::assertNull(R::violation($ordinary, $red, self::ACCOUNTS));
    }
