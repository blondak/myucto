...
final class CompanyBackupPurchaseInvoicePdfTest extends TestCase
{
    public function testPreservesBothLayoutsWhileRemappingTenant(): void
    {
        $policy = CompanyBackupFilePathPolicy::SupplierPurchaseInvoicePdf;
    ...
    public function testSqlSourceKeepsTenantIsolationAndAbsentOptionalPaths(): void
    {
        ...
        $pdo->exec('CREATE TABLE purchase_invoices (id INTEGER PRIMARY KEY, supplier_id INTEGER, pdf_path TEXT)');
        $pdo->exec("INSERT INTO purchase_invoices VALUES (11,7,'supplier-7/ab/abcdef0123456789.pdf'),"
            . "(12,8,'supplier-8/abcdef0123456789.pdf'),(13,7,NULL),(14,7,'')");
        $registry = $this->registry();
        $area = $registry->definition('file-area:purchase-invoice-pdfs');
        ...
        $pdo->exec("UPDATE purchase_invoices SET pdf_path='supplier-8/abcdef0123456789.pdf' WHERE id=11");
        ...
        return new TenantDataRegistry(1, [
            new TenantDataDefinition('table:purchase_invoices', TenantDataObjectKind::Table,
                ($fault === 'policy' ? TenantDataPolicy::TenantOwnedIndirect : TenantDataPolicy::TenantOwned), [$profile], [
                    ...
                    'secrets' => [],
                    'company_backup' => [
                        'data_columns' => ['id', 'supplier_id', 'pdf_path'],
            ...
            new TenantDataDefinition('file-area:purchase-invoice-pdfs', TenantDataObjectKind::FileArea,
                TenantDataPolicy::TenantOwned, [$profile], [
                    ...
                    'file_owners' => [[
                        ...
                        'column' => $fault === 'column' ? 'imported_pdf_path' : 'pdf_path',
                        'path' => [], 'stored_prefix' => $fault === 'prefix' ? 'supplier-7/' : '',
                    ]],
                ]),
        ]);
    }
