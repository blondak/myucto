...
| `cron-shoptet-orders.{cmd,sh}` | Stažení exportu objednávek ze Shoptetu z uloženého odkazu u firem se zapnutým automatickým stahováním; interval si určuje firma |
...
| `test.{sh,ps1}`    | Bez argumentů spustí paralelní PHPUnit nad izolovanými testovacími databázemi. Filter / testsuite předá sériovému PHPUnit (`cmd/test.sh --filter=GpcParser`). Počet workerů lze nastavit přes `php api/bin/test-parallel.php --processes=N`. |
...
| `cron-shoptet-orders` | každých 15 minut (jen firmy se zapnutým stahováním ze Shoptetu) | `*/15 * * * *` |
...
schtasks /create /tn "MyUcto Shoptet Orders" /tr "C:\inetpub\wwwroot\myucto.cz\cmd\cron-shoptet-orders.cmd" /sc minute /mo 15 /ru SYSTEM
...
*/15 *  *   *   *    /var/www/myucto.cz/cmd/cron-shoptet-orders.sh
