# 102. Bezpečnost (MFA, passkeys, zámek session, IP allowlist, role, activity log)
...
> (viz [§ 102.2.4](#10224-obnova-pristupu)). Až když nemáš nic z toho, zbývá CLI
> rescue `php api/bin/reset-mfa.php <email>`.

### 102.2.3 Přihlášení s passkey a MFA
...
### 102.2.6 E-mailové ověření pro účet bez silného faktoru
...
### 102.2.7 Serverový zámek session
...
### 102.2.8 Nasazení změny autentizačního modelu
...
### 102.4.1 Za reverse proxy: `trusted_proxies` (důležité)
...
### 102.4.2 Edge proxy MUSÍ `X-Forwarded-For` přepisovat, ne appendovat
...
### 102.7.2 Jak se do logu zapisuje IP adresa
...
§ 102.4.1). Z hlavičky se bere **první** adresa (původní klient). Bez nastavené
