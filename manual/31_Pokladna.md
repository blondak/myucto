...
promítá do [Účetního deníku](48_Ucetni_denik.md).
...
- **Účet** — analytika účtu 211 z účtové osnovy firmy (např. `211.100` — o tečkovaném
  zápisu viz [§ 84.3.1](84_Ucetni_osnova.md#8431-teckovany-zapis-analytik)). Nabízí se jen aktivní účty
...
### 31.1.2 Správa existujících pokladen
...
## 31.2 Hlavní stránka pokladny — přehled dokladů
...
- **Tlačítka Příjem (PPD)** a **Výdej (VPD)** pro vystavení nového dokladu — zelené pro příjem,
  oranžové pro výdej.
- **Odkaz na Pokladní knihu** (§ 31.5).

### 31.2.1 Filtry a sloupce seznamu
...
### 31.2.2 Přílohy pokladního dokladu
...
- **Tisk** — otevře PDF dokladu (§ 31.4).
- **Storno** — jen u zaúčtovaného dokladu (§ 31.3.5).

Stránka podporuje serverové stránkování (50 dokladů na stránku).

## 31.3 Vystavení pokladního dokladu (PPD/VPD)
...
z editoru faktury volbou způsobu úhrady „Hotově" (§ 31.3.6).

### 31.3.1 Hlavička dokladu
...
partnera. Volitelně lze doklad vystavit s DPH (viz § 31.3.3).
...
### 31.3.3 DPH na pokladním dokladu
...
### 31.3.4 Částka, popis a náhled zaúčtování
...
### 31.3.6 Doklad vzniklý z faktury (hotovostní vyrovnání)
...
pokladně (§ 31.6). Analytika 211 se i tady bere z karty zvolené pokladny.
...
(§ 31.3.2) — pokladna umí i navazující daňový doklad k platbě.

## 31.4 Tisk pokladního dokladu
...
## 31.6 Zaúčtování a vazba na deník

V podvojném účetnictví se každý zaúčtovaný pokladní doklad promítá standardním způsobem do
[Účetního deníku](48_Ucetni_denik.md) — konkrétní účtovací předpis (MD/D) se liší podle účelu
dokladu:
...
Firmy s analytikami DPH ([§ 84.3.2](84_Ucetni_osnova.md#8432-analytiky-dph-343100-343200-a-343900))
...
([§ 84.3.3](84_Ucetni_osnova.md#8433-mesicni-zuctovani-dph)). Firma, která analytiky nemá,
...
- Doklad, jednou zaúčtovaný, se needituje ani nemaže — jedinou opravou je storno s uvedením
  ...
  (§ 31.3.6), který faktura umí zase zrušit.
- Hotovostní vyrovnání z editoru faktury (§ 31.3.6) neumí zálohové faktury, pravidelné
