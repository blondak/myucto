...
u dodavatele** (§ 35.9), **objednávky vydané dodavatelům** (§ 35.11) a návrh
**doplnění zásob** (§ 35.12).

Prodejní objednávky v sekci **Prodej** rezervují skutečně dostupné zboží před
fakturací a předávají rezervaci do vychystání (§ 35.13).
...
(§ 35.11), **U dodavatele** (§ 35.10), **Inventury**, **Sestavy** a
**Intrastat export** (§ 35.15). Číselník
**Sklady** (jednotlivé sklady firmy) je záložka na stránce **E-shop** — modul Sklad
totiž sdílí skladové karty s [e-shopovým modulem](36_Eshop.md) (ceny, kategorie,
...
[§ 95.5 Editace dodavatele](95_Multi_supplier.md#955-editace-dodavatele)) v samostatné
...
### 35.2.1 Detail karty a skladová kniha
...
### 35.2.2 Životní cyklus, duplikace a šablony
...
### 35.2.3 Vazba na e-shopovou kartu
...
dodavatelé) najdeš v kapitole [E-shop](36_Eshop.md).

### 35.2.4 Virtuální sety
...
nejdřív srovnat inventurou (§ 35.7) nebo opravnou příjemkou.
...
### 35.3.1 Příklad výpočtu klouzavého průměru
...
- U druhého příjmu je na řádku zadaná jednotková cena 60,00 Kč (50 ks × 60,00 Kč =
  ...
  viz § 35.4.4), takže do skladu vstoupí celkem 3 200,00 Kč. Nová průměrná cena se
...
- **Zaúčtování** doklad zamkne, přidělí mu **číslo řady** a teprve teď se promítne do
  stavu zásob a skladové knihy karty (podrobně k číslování § 35.4.2). Opakované
...
- **Storno** je možné jen u zaúčtovaného dokladu — storno draftu nebo už jednou
  ...
  pokud na cílovém skladu právě probíhá inventura (§ 35.7) — chybou „Na skladu
...
původem než „Ručně" typicky vznikají automaticky (viz § 35.5 a § 35.7) a v editoru
se u nich zobrazí odkaz na zdrojový doklad.

### 35.4.2 Číslování dokladů
...
číslo. Rozdílové doklady z uzavřené inventury (§ 35.7) čerpají čísla ze **stejné**
běžné řady jako ruční doklady — nemají žádný zvláštní prefix.

### 35.4.3 Editor dokladu
...
### 35.4.4 Vedlejší pořizovací náklady
...
v § 35.3.1, kde druhý příjem obsahuje dopravu 200 Kč).

## 35.5 Vazba na faktury

### 35.5.1 Automatický výdej při vystavení faktury
...
Je-li zapnutá **automatická výdejka** (§ 35.1), při vystavení běžné/finální faktury
...
3. **Rekapitulace** — po uzavření (tlačítko **Uzavřít**, s potvrzovacím dialogem —
   ...
   **rozdílovou příjemku** pro všechny přebytky a/nebo jednu souhrnnou **rozdílovou
   ...
   Číslo dokladu čerpají ze **stejné** řady jako ruční doklady (§ 35.4.2), žádný
...
## 35.9 Kolik toho vlastně máš — skladem, rezervováno, na cestě, u dodavatele
...
|---|---|---|
| **Skladem** (`on_hand`) | fyzický stav na skladě | zaúčtované skladové doklady (§ 35.4) |
| **Rezervováno** (`reserved`) | vyfakturováno zákazníkovi, ale ještě nevydáno ze skladu | řádky vystavených faktur bez zaúčtované výdejky (§ 35.9.2) |
| **Na cestě** (`in_transit`) | objednáno u dodavatele a ještě nedodáno | objednávky u dodavatele (§ 35.11) |
| **U dodavatele** (`at_vendor`) | kolik toho podle svého ceníku drží dodavatel | nabídky dodavatelů (§ 35.10) |
...
ocenění (§ 35.3) ani inventura (§ 35.7) — objednávka ani rezervace se skladovou
knihou nehýbe.

Veličiny se počítají v tisícinách jednotky, stejně jako zbytek skladu, takže na
nich nevzniká zaokrouhlovací chyba.

### 35.9.1 Kde je uvidíš

Na **detailu skladové karty** (§ 35.2.1) jsou nahoře čtyři dlaždice:
...
(REST API `GET /api/stock/quantities` a [MCP server](106_MCP_server.md)),
...
> [!NOTE]
> Náhled dostupnosti u řádku faktury a skladového dokladu (§ 35.4.3, § 35.5.1)
...
> [!IMPORTANT]
> **Firmy se zapnutou automatickou výdejkou (§ 35.1) uvidí v rezervacích trvale
...
(§ 35.10). Nabídka bez vyplněného množství přispěje nulou.
...
## 35.10 Nabídky dodavatelů („u dodavatele")
...
(§ 35.11), pro návrh doplnění zásob (§ 35.12) i pro cenotvorbu e-shopu, která si
z preferovaného dodavatele bere nákupní cenu (viz
[§ 36.8.2](36_Eshop.md#3682-nakupni-cena-cenova-baze)).

Tatáž data se dají editovat i z karty zboží, záložka **Dodavatelé**
([§ 36.9](36_Eshop.md#369-dodavatele-zbozi)) — je to jeden a týž záznam, jen
...
| **Skladem u dodavatele** | množství, které dodavatel hlásí — sčítá se do veličiny *U dodavatele* (§ 35.9.4) |
...
### 35.10.2 Import ceníku dodavatele
...
> (§ 35.10.1) nebo přes [MCP server](106_MCP_server.md), který nabídky umí
> zakládat i upravovat po jedné.

## 35.11 Objednávky u dodavatele
...
(§ 35.9) a zároveň podkladem pro příjemku — zboží z ní naskladníš i dřív, než
přijde faktura.
...
### 35.11.1 Životní cyklus objednávky
...
|---|---|---|
| **Odeslat** | Koncept | Přidělí **číslo řady OBJ** (§ 35.11.3), zapíše okamžik odeslání a od té chvíle se zboží počítá jako **na cestě**. Objednávka bez jediného řádku se odeslat nedá. Opakované kliknutí nic nezkazí — vrátí objednávku beze změny a **další číslo nepropálí**. |
...
| **Zavřít zbytek** | Odesláno, Potvrzeno, Částečně přijato, Přijato | „Zbytek už nedorazí." Nedodané množství se na každém řádku odepíše jako stornované, takže zmizí z „na cestě" a doplnění zásob (§ 35.12) ho zase začne navrhovat k objednání. Koncept se zavírat nedá — ten se maže nebo stornuje. |
...
### 35.11.4 Příjem zboží z objednávky
...
zaúčtuješ standardně ve Skladových dokladech (§ 35.4.1) a teprve tím se zásoba
...
> (§ 35.3) a tím i do ocenění všech následujících výdejů. Není to jen kosmetický
...
## 35.12 Doplnění zásob — co objednat
...
### 35.12.1 Jak se navržené množství počítá
...
- **Balení a minimální odběr** se berou z nabídky **hlavního dodavatele**
  (§ 35.10). Nemá-li karta nabídku, návrh zůstane v holém schodku, bez
  zaokrouhlení.
...
neobjednáváš; nastav mu minimum, nebo objednávej ručně (§ 35.11).
...
### 35.12.3 Jak se k němu dostaneš

Tlačítko **Doplnění zásob** je v hlavičce seznamu objednávek (§ 35.11.5).
...
> „jen pod minimem"** (§ 35.2) — a ten pracuje s **prostým** porovnáním
> „skladem < minimum". Nezná rezervace, neodečítá zboží na cestě a nenavrhuje
> množství. Kompletní výpočet podle § 35.12.1 i **hromadné založení objednávek
...
## 35.13 Vychystání, expedice a vratky
...
## 35.14 Prodejní objednávky a tvrdé rezervace
...
### 35.14.1 Import počátečních zásob
...
## 35.15 Intrastat export pro InstatEvo
...
- Zobrazení karty v e-shopu je nezávislé na jejím skladovém typu — řídí ho
  samostatný příznak **Exportovat do e-shopu** (§ 35.2.3).

### 35.16.1 Co objednávky ještě neumí
...
| **Odeslání objednávky e-mailem** | Tlačítko **Odeslat** znamená „označ za odeslanou", ne „odešli". PDF (§ 35.11.3) stáhni a pošli dodavateli sám. |
| **Automatické přecenění příjemky po doručení faktury** | Nechat příjemku v konceptu, nebo ji po zaúčtování stornovat a přijmout znovu (§ 35.11.4). |
| **Dropshipping** — objednávání zboží až na zakázku | Doplnění zásob pracuje jen s kartami, které mají **minimální zásobu**. Zboží na objednávku objednávej ručně (§ 35.11). Cenotvorbu pro dropshipping popisuje [§ 36.9.2](36_Eshop.md#3692-dropshipping-zbozi-bez-skladu). |
...
| **Obrazovka doplnění zásob a hromadné objednání** | Zatím jen přes API a MCP (§ 35.12.3). |
| **Import ceníku dodavatele přes UI** | V tomto vydání nedoběhne (§ 35.10.2) — zadávej nabídky ručně nebo přes MCP. |
| **Přepnutí „na cestě až od potvrzení" v nastavení** | Pole existuje jen v databázi (§ 35.9.3). |

> [!TIP]
> Skladovou kartu nemusíš mít vždy založenou dopředu — v průvodci **naskladněním
> z přijaté faktury** (§ 35.5.2) ji jde založit rovnou z popisu řádku faktury, bez
...
> dodavatelů (§ 35.10), a teprve podle nich se rozhodnout, jestli a od koho
> objednáš.
