...
u dodavatele** (§ 37.9), **objednávky vydané dodavatelům** (§ 37.11) a návrh
**doplnění zásob** (§ 37.12).

Prodejní objednávky v sekci **Prodej** rezervují skutečně dostupné zboží před
fakturací a předávají rezervaci do vychystání (§ 37.13).
...
(§ 37.11), **U dodavatele** (§ 37.10), **Inventury**, **Sestavy** a
**Intrastat export** (§ 37.15). Číselník
**Sklady** (jednotlivé sklady firmy) je záložka na stránce **E-shop** — modul Sklad
totiž sdílí skladové karty s [e-shopovým modulem](38_Eshop.md) (ceny, kategorie,
...
- **Na PDF faktury rozepsat balení na základní jednotky** *(zobrazí se jen po zapnutí
  ...
  množství v základní jednotce karty (§ 37.5.1).
...
### 37.2.1 Hromadná úprava filtrovaných karet
...
### 37.2.2 Údaje skladové karty pro Intrastat
...
### 37.2.3 Detail karty a skladová kniha
...
### 37.2.4 Životní cyklus, duplikace a šablony
...
### 37.2.5 Vazba na e-shopovou kartu
...
dodavatelé) najdeš v kapitole [E-shop](38_Eshop.md).

### 37.2.6 Virtuální sety
...
([§ 36.17](38_Eshop.md#3817-baleni)), poměr „1 KT = 8 ks" a volitelně **EAN balení**.
...
nejdřív srovnat inventurou (§ 37.7) nebo opravnou příjemkou.
...
### 37.3.1 Příklad výpočtu klouzavého průměru
...
- U druhého příjmu je na řádku zadaná jednotková cena 60,00 Kč (50 ks × 60,00 Kč =
  ...
  viz § 37.4.4), takže do skladu vstoupí celkem 3 200,00 Kč. Nová průměrná cena se
...
- **Zaúčtování** doklad zamkne, přidělí mu **číslo řady** a teprve teď se promítne do
  stavu zásob a skladové knihy karty (podrobně k číslování § 37.4.2). Opakované
...
- **Storno** je možné jen u zaúčtovaného dokladu — storno draftu nebo už jednou
  ...
  pokud na cílovém skladu právě probíhá inventura (§ 37.7) — chybou „Na skladu
...
původem než „Ručně" typicky vznikají automaticky (viz § 37.5 a § 37.7) a v editoru
se u nich zobrazí odkaz na zdrojový doklad.

### 37.4.2 Číslování dokladů
...
číslo. Rozdílové doklady z uzavřené inventury (§ 37.7) čerpají čísla ze **stejné**
běžné řady jako ruční doklady — nemají žádný zvláštní prefix.

### 37.4.3 Editor dokladu
...
### 37.4.4 Vedlejší pořizovací náklady
...
v § 37.3.1, kde druhý příjem obsahuje dopravu 200 Kč).

## 37.5 Vazba na faktury

### 37.5.1 Automatický výdej při vystavení faktury
...
Je-li zapnutá **automatická výdejka** (§ 37.1), při vystavení běžné/finální faktury
...
Řádek fakturovaný v **balení** (§ 37.2.6) se vyskladní v základní jednotce karty:
...
vypneš přepínačem **Na PDF faktury rozepsat balení na základní jednotky** (§ 37.1);
...
skladového dokladu (§ 37.4.4). Po potvrzení vznikne **draft příjemka** (původ
„Přijatá faktura") ve zvoleném skladu a datu, kterou pak podle potřeby doplníš a
zaúčtuješ v modulu Skladové doklady.

Řádek přijaté faktury v **balení** (§ 37.2.6) průvodce nabídne v základní jednotce
...
jednotky nepromítají — k tomu slouží balení karty (§ 37.2.6).
...
3. **Rekapitulace** — po uzavření (tlačítko **Uzavřít**, s potvrzovacím dialogem —
   ...
   **rozdílovou příjemku** pro všechny přebytky a/nebo jednu souhrnnou **rozdílovou
   ...
   Číslo dokladu čerpají ze **stejné** řady jako ruční doklady (§ 37.4.2), žádný
...
|---|---|---|
| **Skladem** (`on_hand`) | fyzický stav na skladě | zaúčtované skladové doklady (§ 37.4) |
| **Rezervováno** (`reserved`) | vyfakturováno zákazníkovi, ale ještě nevydáno ze skladu | řádky vystavených faktur bez zaúčtované výdejky (§ 37.9.2) |
| **Na cestě** (`in_transit`) | objednáno u dodavatele a ještě nedodáno | objednávky u dodavatele (§ 37.11) |
| **U dodavatele** (`at_vendor`) | kolik toho podle svého ceníku drží dodavatel | nabídky dodavatelů (§ 37.10) |
...
ocenění (§ 37.3) ani inventura (§ 37.7) — objednávka ani rezervace se skladovou
knihou nehýbe.

Veličiny se počítají v tisícinách jednotky, stejně jako zbytek skladu, takže na
nich nevzniká zaokrouhlovací chyba.

### 37.9.1 Kde je uvidíš

Na **detailu skladové karty** (§ 37.2.1) jsou nahoře čtyři dlaždice:
...
> [!NOTE]
> Náhled dostupnosti u řádku faktury a skladového dokladu (§ 37.4.3, § 37.5.1)
...
> [!IMPORTANT]
> **Firmy se zapnutou automatickou výdejkou (§ 37.1) uvidí v rezervacích trvale
...
(§ 37.10). Nabídka bez vyplněného množství přispěje nulou.
...
## 37.10 Nabídky dodavatelů („u dodavatele")
...
(§ 37.11), pro návrh doplnění zásob (§ 37.12) i pro cenotvorbu e-shopu, která si
z preferovaného dodavatele bere nákupní cenu (viz
[§ 36.8.2](38_Eshop.md#3882-nakupni-cena-cenova-baze)).

Tatáž data se dají editovat i z karty zboží, záložka **Dodavatelé**
([§ 36.9](38_Eshop.md#389-dodavatele-zbozi)) — je to jeden a týž záznam, jen
...
| **Skladem u dodavatele** | množství, které dodavatel hlásí — sčítá se do veličiny *U dodavatele* (§ 37.9.4) |
...
### 37.10.2 Import ceníku dodavatele
...
> (§ 37.10.1) nebo přes [MCP server](106_MCP_server.md), který nabídky umí
> zakládat i upravovat po jedné.

## 37.11 Objednávky u dodavatele
...
(§ 37.9) a zároveň podkladem pro příjemku — zboží z ní naskladníš i dřív, než
přijde faktura.
...
### 37.11.1 Životní cyklus objednávky
...
|---|---|---|
| **Odeslat** | Koncept | Přidělí **číslo řady OBJ** (§ 37.11.3), zapíše okamžik odeslání a od té chvíle se zboží počítá jako **na cestě**. Objednávka bez jediného řádku se odeslat nedá. Opakované kliknutí nic nezkazí — vrátí objednávku beze změny a **další číslo nepropálí**. |
...
| **Zavřít zbytek** | Odesláno, Potvrzeno, Částečně přijato, Přijato | „Zbytek už nedorazí." Nedodané množství se na každém řádku odepíše jako stornované, takže zmizí z „na cestě" a doplnění zásob (§ 37.12) ho zase začne navrhovat k objednání. Koncept se zavírat nedá — ten se maže nebo stornuje. |
...
### 37.11.4 Příjem zboží z objednávky
...
zaúčtuješ standardně ve Skladových dokladech (§ 37.4.1) a teprve tím se zásoba
...
> (§ 37.3) a tím i do ocenění všech následujících výdejů. Není to jen kosmetický
...
## 37.12 Doplnění zásob — co objednat
...
### 37.12.1 Jak se navržené množství počítá
...
- **Balení a minimální odběr** se berou z nabídky **hlavního dodavatele**
  (§ 37.10). Nemá-li karta nabídku, návrh zůstane v holém schodku, bez
  zaokrouhlení.
...
neobjednáváš; nastav mu minimum, nebo objednávej ručně (§ 37.11).
...
### 37.12.3 Jak se k němu dostaneš
...
## 37.14 Prodejní objednávky a tvrdé rezervace
...
### 37.14.1 Import počátečních zásob
...
## 37.15 Intrastat export pro InstatEvo
...
- Zobrazení karty v e-shopu je nezávislé na jejím skladovém typu — řídí ho
  samostatný příznak **Exportovat do e-shopu** (§ 37.2.3).

### 37.16.1 Co objednávky ještě neumí
...
| **Odeslání objednávky e-mailem** | Tlačítko **Odeslat** znamená „označ za odeslanou", ne „odešli". PDF (§ 37.11.3) stáhni a pošli dodavateli sám. |
| **Automatické přecenění příjemky po doručení faktury** | Nechat příjemku v konceptu, nebo ji po zaúčtování stornovat a přijmout znovu (§ 37.11.4). |
...
> z přijaté faktury** (§ 37.5.2) ji jde založit rovnou z popisu řádku faktury, bez
...
> dodavatelů (§ 37.10), a teprve podle nich se rozhodnout, jestli a od koho
> objednáš.
