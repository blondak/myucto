...
Související výkazy a exporty mají v manuálu vlastní kapitoly: [Kniha DPH](40_Kniha_DPH.md)
(interní žurnál), [Souhrnné hlášení](42_Souhrnne_hlaseni.md) (EU dodání B2B) a
[Hromadný export](45_Hromadny_export.md) (ZIP balíček pro účetní). Rozdíl mezi
staženým a skutečně podaným XML vysvětluje [Archiv podání a daňová
rekonciliace](93_Archiv_podani_a_rekonciliace.md). Výkazy najdeš v menu
...
export popisuje oddíl [OSS přiznání](#393-oss-priznani-ossei1).

## 39.1 Předpoklady před prvním podáním
...
Detailní mapping všech polí v UI na XML atributy najdeš v sekci [Pole EPO / VetaP](#392-pole-epo-vetap) níže.
...
### 39.2.1 Identifikace finančního úřadu
...
### 39.2.2 Typ plátce a perioda

| Pole v UI | XML atribut | Hodnoty | Kdy použít |
|---|---|---|---|
| **Typ poplatníka** | `typ_ds` ve VetaP | `F` (FO/OSVČ) / `P` (PO/s.r.o.) | Podle právní formy. |
| **Typ plátce DPH** | `typ_platce` ve VetaD | `P` (plátce) / `I` (identifikovaná osoba) | `I` se nastaví automaticky, když je firma k rozhodnému datu vedená v historii plátcovství jako **Identifikovaná osoba** (viz [§ 38.1.4](38_Fakturujeme.md#3814-identifikovana-osoba-6g-6l-zdph)). Perioda (měsíc/kvartál) jde zvlášť atributy `mesic`/`ctvrt` dle `vat_period`. |
...
### 39.2.4 Osobní údaje (jen pro FO/OSVČ)
...
### 39.2.5 Oprávněná osoba k podpisu — POVINNÉ pro PO
...
### 39.2.6 Sestavitel přiznání (sest_*)
...
### 39.2.7 Kontaktní údaje pro podání
...
### 39.2.8 Postup podání na EPO portál
...
[§ 43.8.5](43_OSS.md#4395-kde-se-oss-priznani-podava).
...
> Shop)](43_OSS.md)**: nastavení a registrace, odvození řádku, plnění k ručnímu
...
### 39.3.1 Co se z OSS promítne do přiznání k DPH
...
vysvětluje [§ 43.4](43_OSS.md#435-plneni-k-rucnimu-posouzeni).

Účtování OSS daně na vlastní účet **345.100** je důvod, proč **zůstatek 343 jde
s přiznáním k DPH srovnat** — podrobně
[§ 43.7](43_OSS.md#438-uctovani-oss-dane).

## 39.4 DPH přiznání (DPHDP3)

### 39.4.1 Cesta: `Daně → DPH přiznání`
...
### 39.4.2 Převod DPH na zúčtovací účet
...
### 39.4.3 Jak se DPHDP3 generuje a co zahrnuje
...
> IO podává vždy měsíčně. Podrobnosti [§ 38.1.4](38_Fakturujeme.md#3814-identifikovana-osoba-6g-6l-zdph).
...
administrátorské API** — obdobně jako [zámek účtování k datu](48_Ucetni_denik.md#489-zamek-uctovani-k-datu),
...
- Samotné **účetní zaúčtování ročního vypořádání** (na účty 548/343, u firem
  ...
  [Účetního deníku](48_Ucetni_denik.md) ji zapiš ručním zápisem. Nezaměňuj to
  s [měsíčním zúčtováním DPH](84_Ucetni_osnova.md#8433-mesicni-zuctovani-dph),
...
### 39.4.4 Jak fungují VAT klasifikační kódy
...
### 39.4.5 Auto-default klasifikace
...
### 39.4.6 Override per řádek nebo header
...
### 39.4.7 Reverse charge v cizí měně
...
### 39.4.8 Pořízení dlouhodobého majetku
...
Daň se v součtech ř. 46 neduplikuje (ř. 47 je informativní). V [Knize DPH](40_Kniha_DPH.md)
je samostatná sekce **47.047** se sumací.

## 39.5 Kontrolní hlášení (DPHKH1)

### 39.5.1 Cesta: `Daně → Kontrolní hlášení`
...
### 39.5.2 Typ podání — řádné, opravné, následné
...
### 39.5.3 Pravidla zařazení do sekcí
...
### 39.5.4 Zvláštní režimy a opravy nedobytných pohledávek
...
### 39.7.2 §79 a §79a — registrace a zrušení registrace
...
## 39.9 Změna sazby DPH s budoucí platností
...
### 39.10.1 "Chybí kód finančního úřadu"
→ Doplň v Nastavení → Daňové nastavení.

### 39.10.2 "Faktura nemá VAT klasifikační kód"
...
### 39.10.3 "DIČ klienta není ve formátu CZxxxxxxxx"
...
### 39.10.4 "Dodatečné přiznání vyžaduje datum zjištění důvodů"
...
### 39.10.5 "Pro dané období neexistuje dřívější řádné/opravné přiznání"
...
### 39.10.6 "Opravné dodatečné přiznání (druh E) zatím není podporováno"
...
## 39.11 Podpora pro daňového poradce
