...
— firmy na [daňové evidenci](69_Danova_evidence.md) vedou místo něj jednodušší
[Peněžní deník](69_Danova_evidence.md) bez podvojných zápisů.
...
> [Účtový rozvrh](60_Ucetni_osnova.md) a
> [Předkontace](67_Ucetni_nastroje.md#673-predkontace). Tato kapitola popisuje
...
| **Kurzové přecenění** | přecenění cizoměnových zůstatků k rozvahovému dni (viz [§ 44.5](#445-multi-menove-radky-zapisu)) |
| **Ruční** | zápis, který jsi založil(a) přímo v deníku (viz [§ 44.4](#444-rucni-zapis)) |
...
- přepis se **neprovede** u zápisu, který je mezitím **stornovaný** (viz
  [§ 44.8](#448-storno-a-oprava-zauctovaneho-zapisu)) — takový zápis se z principu už
...
- tlačítko **Kopírovat jako nový** — otevře formulář **Ruční účetní zápis** s předvyplněnými
  ...
  opakují bez šablony (jednorázová varianta oproti šablonám, viz [§ 44.4](#444-rucni-zapis)),
...
## 44.3 Koncept vs. zaúčtovaný zápis
...
přepisovatelných dokladů (viz idempotence v [§ 44.1](#idempotence-proc-doklad-nejde-zauctovat-dvakrat))
...
- **Číslo dokladu** — volitelné; necháš-li prázdné a firma má na stránce
  [Účetní období](66_Uzaverka.md) zapnutou volbu **„Automatická čísla dokladů ručních
...
- **Částku** — kladné číslo, na 2 desetinná místa (vždy v účetní měně CZK — ruční zápis
  ...
  automatických zápisů z cizoměnových faktur, viz [§ 44.5](#445-multi-menove-radky-zapisu)),
...
číselných řad jako řada pro ruční zápisy — viz [Účetní období](66_Uzaverka.md)). Zadáš:
...
Superadmin zde má navíc podzáložku **Šablony banky**. Ta spravuje globální katalog
pravidel nabízených všem firmám přes **Banka → Pravidla účtování → Ze šablony**.
...
## 44.5 Multi-měnové řádky zápisu
...
v CZK (viz [§ 44.2](#rozklik-na-detail-zapisu)), takže u faktury v eurech vidíš
zaokrouhlenou korunovou částku i skutečnou částku v EUR, ze které vznikla.

### Kurzové přecenění a kurzové rozdíly

K rozvahovému dni (v rámci [uzávěrky](66_Uzaverka.md)) systém přecení otevřené
...
> (odlišný od Kapitálu) — viz [Účtový rozvrh](60_Ucetni_osnova.md) a
> [Předkontace](67_Ucetni_nastroje.md#673-predkontace).
> Díky tomu je výkazy (rozvaha/VZZ) do vlastního kapitálu firmy nezapočítávají.

## 44.6 Popis, přílohy a poznámky
...
## 44.7 Historie zápisu

Každý přepis existujícího zápisu (idempotentní re-post popsaný v
[§ 44.1](#idempotence-proc-doklad-nejde-zauctovat-dvakrat)) i editace popisu
...
## 44.8 Storno a oprava zaúčtovaného zápisu
...
- **Otevřené období — oprava přepisem.** Když opravíš údaje na zdrojovém dokladu
  ...
  drží auditní stopa na úrovni databáze (viz [§ 44.7](#447-historie-zapisu)),
...
- **Zaúčtovaný zápis, který přepisem opravovat nechceš (nebo nejde) — storno.**
  ...
  **jednou** — opakované storno hlásí „Zápis už byl stornován" (i kdyby dva požadavky
  ...
  [§ 44.3](#443-koncept-vs-zauctovany-zapis)) a jen do otevřeného období.
...
- Právě proto, že se protizápis snaží zaúčtovat do období originálu: pokud je
  ...
  přes [Uzávěrku](66_Uzaverka.md), nebo doklad neuzavíráš, ale opravíš
  přeúčtováním.
...
> [Uzávěrku](66_Uzaverka.md) (znovuotevření knih do schválení závěrky), ne přímé storno
> v deníku.
...
## 44.9 Zámek účtování k datu

Kromě uzavření **celého** účetního období (viz [Uzávěrka](66_Uzaverka.md)) existuje i
...
**administrátor** — na stránce [**Účetnictví → Měsíční kontrola**](54_Mesicni_kontrola.md)
...
## 44.10 Kontrola integrity deníku (noční job)
...
- Zaúčtovat (i stornovat, i přepsat idempotentním re-postem) lze jen do **otevřeného**
  účetního období — nejdřív si ověř v [Uzávěrce](66_Uzaverka.md), že období pro dané
...
- Kromě otevřenosti období hlídá zaúčtování/přeúčtování/storno i **zámek k datu**
  (§ 44.9). Po doloženém podání jej nastav administrátor; backend jej umí posunout také
