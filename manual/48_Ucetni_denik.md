...
— firmy na [daňové evidenci](94_Danova_evidence.md) vedou místo něj jednodušší
[Peněžní deník](94_Danova_evidence.md) bez podvojných zápisů.
...
> [Účtový rozvrh](84_Ucetni_osnova.md) a
> [Předkontace](92_Ucetni_nastroje.md#923-predkontace). Tato kapitola popisuje
...
> ([§ 48.12](#4812-douctovani-nezauctovanych-dokladu)).

## 48.1 Odkud se zápisy berou
...
|---|---|
| **Vydaná faktura** | zaúčtování vydané faktury (311/6xx + DPH na výstupu **343.200** podle [Knihy DPH](40_Kniha_DPH.md)) |
...
| **Pokladna** | zaúčtování pokladního dokladu — viz [Pokladna](31_Pokladna.md) |
...
| **Zúčtování DPH** | měsíční (u čtvrtletního plátce čtvrtletní) interní doklad, který převede vstupní a výstupní daň na **343.900** — viz [§ 84.3.3](84_Ucetni_osnova.md#8433-mesicni-zuctovani-dph) |
| **Kurzové přecenění** | přecenění cizoměnových zůstatků k rozvahovému dni (viz [§ 48.5](#485-multi-menove-radky-zapisu)) |
| **Ruční** | zápis, který jsi založil(a) přímo v deníku (viz [§ 48.4](#484-rucni-zapis)) |
...
### 48.1.1 Idempotence — proč doklad nejde zaúčtovat dvakrát
...
- přepis se **neprovede** u zápisu, který je mezitím **stornovaný** (viz
  [§ 48.8](#488-storno-a-oprava-zauctovaneho-zapisu)) — takový zápis se z principu už
...
- **Zdroj** — typ a číslo zdrojového dokladu; odznak **Automaticky** se u
  ...
  [Souvisí: doklad a jeho úhrada](#4822-souvisi-doklad-a-jeho-uhrada)),
...
### 48.2.1 Drill-down na zdrojový doklad
...
- **pokladna** — seznam **Pokladna** předfiltrovaný na konkrétní pokladnu a číslo
  ...
  najde — viz [Pokladna](31_Pokladna.md)).
...
### 48.2.2 Souvisí: doklad a jeho úhrada
...
[Vazba na doklad](#4864-vazba-na-doklad)) — mají vlastní barvu štítku, protože nevznikly
...
- **Zdroj** — omezení na jeden typ zdroje, včetně **Banka** a **Pokladna** (drill-down
  z [Banky](28_Banka.md)/[Pokladny](31_Pokladna.md) i tento filtr vedou ke stejnému
...
### 48.2.5 Rozklik na detail zápisu
...
- tlačítko **Kopírovat jako nový** — otevře formulář **Ruční účetní zápis** s předvyplněnými
  ...
  opakují bez šablony (jednorázová varianta oproti šablonám, viz [§ 48.4](#484-rucni-zapis)),
...
## 48.3 Koncept vs. zaúčtovaný zápis
...
přepisovatelných dokladů (viz idempotence v [§ 48.1](#4811-idempotence-proc-doklad-nejde-zauctovat-dvakrat))
...
- **Číslo dokladu** — volitelné; necháš-li prázdné a firma má na stránce
  [Účetní období](91_Uzaverka.md) zapnutou volbu **„Automatická čísla dokladů ručních
...
- **Částku** — kladné číslo, na 2 desetinná místa (vždy v účetní měně CZK — ruční zápis
  ...
  automatických zápisů z cizoměnových faktur, viz [§ 48.5](#485-multi-menove-radky-zapisu)),
...
### 48.4.4 Nejčastější chybové hlášky při ukládání
...
### 48.4.5 Příklad: ruční zápis se dvěma řádky
...
### 48.4.6 Převod mezi účty (261 — Peníze na cestě)
...
číselných řad jako řada pro ruční zápisy — viz [Účetní období](91_Uzaverka.md)). Zadáš:
...
### 48.4.8 Šablony ručních zápisů a mzdový můstek
...
## 48.5 Multi-měnové řádky zápisu
...
v CZK (viz [§ 48.2](#4825-rozklik-na-detail-zapisu)), takže u faktury v eurech vidíš
zaokrouhlenou korunovou částku i skutečnou částku v EUR, ze které vznikla.

### 48.5.1 Kurzové přecenění a kurzové rozdíly

K rozvahovému dni (v rámci [uzávěrky](91_Uzaverka.md)) systém přecení otevřené
...
> (odlišný od Kapitálu) — viz [Účtový rozvrh](84_Ucetni_osnova.md) a
...
vlastní přílohy v [Dokumentech](32_Dokumenty.md). Přílohy zápisu se ukládají do
...
Vazbu jde založit i rovnou při zakládání [ručního zápisu](#484-rucni-zapis) — sekce
...
## 48.7 Historie zápisu

Každý přepis existujícího zápisu (idempotentní re-post popsaný v
[§ 48.1](#4811-idempotence-proc-doklad-nejde-zauctovat-dvakrat)) i editace popisu
...
## 48.8 Storno a oprava zaúčtovaného zápisu
...
- **Otevřené období — oprava přepisem.** Když opravíš údaje na zdrojovém dokladu
  ...
  drží auditní stopa na úrovni databáze (viz [§ 48.7](#487-historie-zapisu)),
...
- **Zaúčtovaný zápis, který přepisem opravovat nechceš (nebo nejde) — storno.**
  ...
  **jednou** — opakované storno hlásí „Zápis už byl stornován" (i kdyby dva požadavky
  ...
  [§ 48.3](#483-koncept-vs-zauctovany-zapis)) a jen do otevřeného období.
...
- **Přeúčtování z dokladu.** Všechny tři cesty výš (přepis, storno, odmítnutí) má pod
  jedním tlačítkem i doklad sám — viz [§ 48.8.2](#4882-preuctovani-z-dokladu-sekce-zauctovani).

### 48.8.1 Automatické storno při smazání nebo interním stornu dokladu
...
- Právě proto, že se protizápis snaží zaúčtovat do období originálu: pokud je
  ...
  přes [Uzávěrku](91_Uzaverka.md), nebo doklad neuzavíráš, ale opravíš
  přeúčtováním.
...
> [Uzávěrku](91_Uzaverka.md) (znovuotevření knih do schválení závěrky), ne přímé storno
> v deníku.
...
### 48.8.2 Přeúčtování z dokladu (sekce Zaúčtování)
...
| otevřený rok, datum pod [zámkem k datu](#489-zamek-uctovani-k-datu), oprava jen přesouvá částky mezi účty téže třídy | původní zápis se **přepíše na místě** k původnímu datu, bez storna (podmínky viz níž) |
...
## 48.9 Zámek účtování k datu

Kromě uzavření **celého** účetního období (viz [Uzávěrka](91_Uzaverka.md)) existuje i
...
(viz [Výkazy DPH](39_Vykazy_DPH.md)) samo zámek neposouvá. Backend jej posune až při
...
**administrátor** — na stránce [**Účetnictví → Měsíční kontrola**](58_Mesicni_kontrola.md)
...
## 48.10 Kontrola integrity deníku (noční job)
...
- Zaúčtovat (i stornovat, i přepsat idempotentním re-postem) lze jen do **otevřeného**
  účetního období — nejdřív si ověř v [Uzávěrce](91_Uzaverka.md), že období pro dané
...
- Kromě otevřenosti období hlídá zaúčtování/přeúčtování/storno i **zámek k datu**
  (§ 48.9). Po doloženém podání jej nastav administrátor; backend jej umí posunout také
...
## 48.12 Doúčtování nezaúčtovaných dokladů
