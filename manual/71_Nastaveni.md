...
(typicky `EUR`) a volitelně datum začátku a konce registrace. Po zapnutí se
v editoru položek zobrazí OSS pole a v menu **Daně → OSS přiznání** kvartální
přehled a XML export.

OSS režim sám neurčuje, které plnění do OSS patří. Každý řádek musí uživatel
označit a doplnit jeho zemi spotřeby, typ sazby a typ plnění. Aplikace
