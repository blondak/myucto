# 72. Více dodavatelů z jedné instalace
...
## 72.2 Přidání nového dodavatele
...
### 72.2.1 Modal nového dodavatele
...
## 72.3 Co je per-dodavatel (izolované)
...
- **Šablonu čísla faktury** — vlastní formát per typ dokladu (`{YY}{MM}{CCC}`,
  `JD{YYYY}-{CC}`, …) + reset cyklu (rok / měsíc / nikdy) — viz § 72.5.3
...
## 72.4 Co je sdílené (cross-supplier)
...
## 72.6 Režim účtování per firma

Kromě izolovaných dat (§ 72.3) si každý dodavatel v multi-supplier instalaci **nezávisle
na ostatních** volí i svůj vlastní **režim účetnictví**. Přepnutí dodavatele ve spodní liště
(§ 72.1) tak nemění jen viditelná data, ale i to, jaké sekce menu a moduly máš k dispozici —
...
### 72.6.1 Kde se nastavuje

V **detailu dodavatele** (§ 72.5), sekce **„Daňové nastavení"** (stejný box jako EPO údaje
a Pohoda kódy, viz [§ 72.5.6](#7256-pohoda-kody)), je pole **Režim účetnictví**:
...
> [!NOTE]
> Pole smí měnit jen **admin**, stejně jako ostatní údaje v detailu dodavatele (§ 72.5).
> Výjimkou je zapnutí skladu (§ 72.6.3), které smí přepnout i role **účetní**.
...
> Podrobný postup je v [§ 71.10.3](71_Danova_evidence.md#71103-pruvodce-aktivaci-podvojneho-ucetnictvi).
...
> [Daňová evidence § 51.10](71_Danova_evidence.md).
...
### 72.6.2 Co která volba zpřístupní v menu

Volba se v menu projeví **okamžitě** po přihlášení nebo po přepnutí dodavatele (§ 72.1):
...
[Daňová evidence](71_Danova_evidence.md) pro daňovou evidenci.
...
### 72.6.3 Sklad je nezávislý na režimu účetnictví
...
> z jinak admin-only nastavení dodavatele (§ 72.5).

## 72.7 Smazání dodavatele
...
## 72.9 Přehled firem (pro účetní kancelář)

Přepínání dodavatele (§ 72.1) stačí, dokud spravuješ pár firem. Účetní kancelář
...
firmu** (stejný mechanismus jako přepínač v horní liště, § 72.1) a rovnou tě
