...
mezi kterými se přepíná v nastavení dodavatele (viz [Multi_supplier](95_Multi_supplier.md)).
Firma v režimu „Podvojné účetnictví" místo této sekce vidí plnohodnotné **Účetnictví**
(deník, hlavní kniha, rozvaha, výsledovka) popsané v [§ 44 a násl.](48_Ucetni_denik.md).
...
> (viz [§ 94.6](#946-nezarazene-pohyby-a-varovani)).
...
proto samo nesmí změnit výsledek DPH. Viz [Výkazy DPH](39_Vykazy_DPH.md).

## 94.1 Zapnutí režimu a dostupnost v menu
...
[Multi_supplier](95_Multi_supplier.md)). Podle aktuálně zvoleného režimu se v hlavním
...
> doklady (PPD/VPD) — viz [Pokladna](31_Pokladna.md). V daňové evidenci ale doklad
...
## 94.2 Peněžní deník — filtry a rozsah
...
## 94.3 Souhrn (počáteční a konečný zůstatek, totály)
...
| Nezařazeno | *(zobrazí se jen když existuje)* — pohyby čekající na zařazení, viz § 94.6 |
...
[kapitole Daň z příjmů](41_Dan_z_prijmu.md#412-jak-se-pocita-dpfo).
...
| Klasifikace | Barevný štítek s daňovým zařazením (viz § 94.5) |
...
## 94.5 Daňová klasifikace pohybů
...
| **Nezařazeno** | Pohyb, který systém nedokázal automaticky zařadit — viz § 94.6 |
...
> Ostatní — viz [§ 31.3.2 Účel dokladu](31_Pokladna.md#3132-ucel-dokladu)).
...
## 94.6 Nezařazené pohyby a varování
...
## 94.7 Kontrola vůči přiznanému příjmu
...
### 94.8.2 Tabulky pohledávek a závazků
...
[§ 26](26_Platebni_prikazy.md) a [§ 31.3.2](31_Pokladna.md#3132-ucel-dokladu)).
...
### 94.8.3 Roční uzávěrka daňové evidence
...
## 94.9 Návaznost na ostatní moduly

- **Pokladna** ([§ 29](31_Pokladna.md)) — jediné místo, kde v režimu daňové
...
- **Daň z příjmů (DPFO)** ([samostatná kapitola](41_Dan_z_prijmu.md)) — u OSVČ v režimu daňové
...
- **Více dodavatelů** ([§ 52](95_Multi_supplier.md)) — režim účetnictví (daňová
...
## 94.10 Přechodový můstek mezi evidencí a účetnictvím (přílohy č. 2 a 3 ZDP)

Když se firma vedená v daňové evidenci rozhodne přejít na **podvojné účetnictví**
(změna režimu, viz [§ 95.6.1](95_Multi_supplier.md)), nejde jen o
...
### 94.10.1 Co se do úpravy počítá
...
### 94.10.2 Co systém nabízí — podklady pro přechod
...
> (viz [Daň z příjmů](41_Dan_z_prijmu.md)).
...
### 94.10.3 Průvodce aktivací podvojného účetnictví
...
- Sestavy jsou z většiny **read-only** — v peněžním deníku ani v přehledu
  ...
  pokladních pohybů bez navázaného dokladu ([§ 94.6](#946-nezarazene-pohyby-a-varovani))
