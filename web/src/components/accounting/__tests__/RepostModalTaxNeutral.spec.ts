...
async function editLines(wrapper: Awaited<ReturnType<typeof mountModal>>, lines: Array<{ account_code: string; side: 'debit' | 'credit'; amount: number; is_red_storno?: boolean }>) {
...
describe('RepostModal — přepis na místě v zamčeném datu', () => {
  ...
  it('změna červeného znaménka spustí nový plán a předá příznaky řádků', async () => {
    m.plan = { ...lockedPlan(), lines: ORIGINAL.map(line => ({ ...line, is_red_storno: true })) }
    m.linesPlan = { ...lockedPlan(), tax_neutral_violation: 'amounts_changed' }
    const wrapper = await mountModal()
    await editLines(wrapper, [
      { ...ORIGINAL[0], is_red_storno: false },
      { ...ORIGINAL[1], is_red_storno: true },
      { ...ORIGINAL[2], is_red_storno: true },
    ])

    expect(m.planForLines).toHaveBeenCalledTimes(1)
    expect(m.planForLines.mock.calls[0][2]).toEqual([
      { account_code: '518.100', side: 'debit', amount: 1000, is_red_storno: false },
      { account_code: '343.100', side: 'debit', amount: 210, is_red_storno: true },
      { account_code: '321.100', side: 'credit', amount: 1210, is_red_storno: true },
    ])
    wrapper.unmount()
  })
})
