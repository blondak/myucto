      ...
      <table class="w-full text-sm" data-test="cash-flow-table">
        <tbody class="divide-y divide-neutral-100">
          <template v-for="line in lines" :key="line.key">
            <tr :class="[line.strong ? 'font-semibold bg-neutral-50/60' : '', line.group?.accounts.length ? 'cursor-pointer hover:bg-neutral-50' : '']"
                :data-test="`cf-${line.key}`"
                @click="line.group?.accounts.length ? toggle(line.key) : undefined">
              <td class="px-4 py-2">
                <span v-if="line.group?.accounts.length" class="inline-block mr-1 text-neutral-400 transition-transform"
                      :class="{ 'rotate-90': expanded.has(line.key) }">▸</span>
                {{ line.label }}
              </td>
              <td class="px-4 py-2 text-right tabular-nums whitespace-nowrap" :class="{ 'text-danger-600': line.amount < 0 }">{{ money(line.amount) }}</td>
            </tr>
            <template v-if="line.group && expanded.has(line.key)">
              <tr v-for="acc in line.group.accounts" :key="`${line.key}-${acc.account_code}`" class="text-neutral-600" data-test="cf-account">
