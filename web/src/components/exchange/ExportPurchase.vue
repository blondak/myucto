...
type Format = 'pdf-zip' | 'pohoda' | 'isdoc' | 'csv'
...
<template>
  <div class="max-w-3xl space-y-4">
    <p class="text-sm text-neutral-500">{{ t('purchase_invoice.export.page_subtitle') }}</p>
    ...
    <div class="bg-surface border border-neutral-200 rounded-lg p-5 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
          <label
            v-for="opt in [
              ...
              { val: 'csv' as Format,     label: t('purchase_invoice.export.fmt_csv'),    hint: t('purchase_invoice.export.fmt_csv_hint') },
            ]"
            :key="opt.val"
      ...
      <div v-else-if="format === 'csv'" class="rounded-md bg-primary-50 border border-primary-200 px-3 py-2 text-sm text-primary-700">
        <strong>{{ t('purchase_invoice.export.fmt_csv') }}:</strong>
        {{ t('purchase_invoice.export.csv_info_hint') }}
      </div>
