import { PAYROLL_MANUAL_CHAPTERS } from '@/config/payrollManualChapters'

export type ManualChapterRule = [RegExp, string]

export const MANUAL_CHAPTERS: ManualChapterRule[] = [
  [/^\/imports\/money-s3(?:\/|$)/, '83a_Prechod_z_Money_S3'],
  [/^\/accounting\/setup-assistant(?:\/|$)/, '80_Sablony'],
  [/^\/admin\/bank-rule-templates(?:\/|$)/, '80_Sablony'],
  [/^\/templates(?:\/|$)/, '80_Sablony'],
  [/^\/purchase-invoices\/payment-orders(?:\/|$)/, '26_Platebni_prikazy'],
  [/^\/purchase-invoices\/ai-import(?:\/|$)/, '25_AI_extrakce'],
  [/^\/purchase-invoices\/export(?:\/|$)/, '24_Export_prijatych'],
  [/^\/purchase-invoices\/import(?:\/|$)/, '21_Importy'],
  [/^\/purchase-invoices(?:\/|$)/, '23_Prijate_faktury'],
  [/^\/invoices\/ai-import(?:\/|$)/, '21_Importy'],
  [/^\/invoices\/export(?:\/|$)/, '20_Exporty'],
  [/^\/invoices\/import(?:\/|$)/, '21_Importy'],
  [/^\/invoices\/new(?:\/|$)/, '15_Faktura_editor'],
  [/^\/invoices\/\d+(?:\/|$)/, '16_Faktura_PDF'],
  [/^\/invoices(?:\/|$)/, '14_Faktury'],
  [/^\/recurring(?:\/|$)/, '17_Pravidelne_fakturace'],
  [/^\/clients(?:\/|$)/, '18_Klienti'],
  [/^\/projects(?:\/|$)/, '19_Zakazky'],
  [/^\/bank(?:\/|$)/, '28_Banka'],
  [/^\/gopay(?:\/|$)/, '29a_GoPay'],
  [/^\/accounting\/cash(?:\/|$)/, '30_Pokladna'],
  [/^\/documents(?:\/|$)|^\/document-requests(?:\/|$)/, '31_Dokumenty'],
  [/^\/logbook(?:\/|$)/, '32_Kniha_jizd'],
  [/^\/stock(?:\/|$)/, '33_Sklad'],
  [/^\/eshop(?:\/|$)/, '34_Eshop'],
  [/^\/reports\/dph-book(?:\/|$)/, '37_Kniha_DPH'],
  [/^\/reports\/shv(?:\/|$)/, '39_Souhrnne_hlaseni'],
  [/^\/reports\/oss(?:\/|$)/, '40_OSS'],
  [/^\/reports\/income-tax(?:\/|$)/, '38_Dan_z_prijmu'],
  // Oznámení podle § 38da a § 38e je popsané v kapitole o dani z příjmů,
  // ne u mezd: mzdových příjmů se obě podání netýkají.
  [/^\/reports\/foreign-income(?:\/|$)/, '38_Dan_z_prijmu'],
  [/^\/reports\/submissions(?:\/|$)/, '89_Archiv_podani_a_rekonciliace'],
  [/^\/reports\/monthly-export(?:\/|$)/, '42_Hromadny_export'],
  [/^\/reports\/(?:dph|kh|s74b|vat-corrections|vat-coefficient|s46)(?:\/|$)/, '36_Vykazy_DPH'],
  [/^\/reports\/(?:cnb-rate-audit|invoice-series-completeness)(?:\/|$)/, '79_Ucetni_kontroly_a_inventarizace'],
  [/^\/tax(?:\/|$)/, '41_Danovy_optimalizator'],
  [/^\/portfolio(?:\/|$)/, '44_Prehled_firem'],
  [/^\/automation(?:\/|$)/, '46_Automat'],
  [/^\/accounting\/manual-posting-queue(?:\/|$)/, '47_Rucni_fronta_doctovani'],
  [/^\/accounting\/general-ledger(?:\/|$)/, '48_Hlavni_kniha'],
  [/^\/accounting\/trial-balance(?:\/|$)/, '49_Obratova_predvaha'],
  [/^\/accounting\/balance-sheet(?:\/|$)/, '50_Rozvaha'],
  [/^\/accounting\/statement-mapping(?:\/|$)/, '50_Rozvaha'],
  [/^\/accounting\/income-statement-by-function(?:\/|$)/, '52_Vysledovka_ucelova'],
  [/^\/accounting\/income-statement(?:\/|$)/, '51_Vysledovka_druhova'],
  [/^\/accounting\/saldo(?:\/|$)/, '53_Saldokonto'],
  [/^\/accounting\/document-completeness(?:\/|$)/, '54_Uplnost_dokladu'],
  [/^\/accounting\/monthly-check(?:\/|$)/, '55_Mesicni_kontrola'],
  [/^\/accounting\/monthly-report(?:\/|$)/, '56_Mesicni_report'],
  ...PAYROLL_MANUAL_CHAPTERS,
  [/^\/accounting\/payroll(?:\/|$)/, '57_Mzdy'],
  [/^\/accounting\/assets(?:\/|$)|^\/accounting\/small-assets(?:\/|$)/, '78_Majetek'],
  [/^\/accounting\/accounts(?:\/|$)/, '81_Ucetni_osnova'],
  [/^\/accounting\/offsets(?:\/|$)/, '82_Zapocty'],
  [/^\/admin\/accounting-activation(?:\/|$)/, '83_Aktivace_ucetnictvi'],
  [/^\/accounting\/balance-inventory(?:\/|$)/, '84_Inventarizace_rozvahovych_uctu'],
  [/^\/accounting\/section18-statements(?:\/|$)/, '85_Vykazy_podle_paragrafu_18'],
  [/^\/reports\/related-parties(?:\/|$)/, '86_Propojene_osoby'],
  [/^\/accounting\/periods(?:\/|$)/, '87_Uzaverka'],
  [/^\/accounting\/journal(?:\/|$)/, '45_Ucetni_denik'],
  [/^\/accounting(?:\/|$)|^\/utilities(?:\/|$)/, '88_Ucetni_nastroje'],
  [/^\/tax-evidence(?:\/|$)/, '90_Danova_evidence'],
  [/^\/admin\/suppliers(?:\/|$)/, '91_Multi_supplier'],
  [/^\/payment-cards(?:\/|$)/, '92a_Platebni_karty'],
  [/^\/admin\/databox(?:\/|$)/, '93_Datova_schranka'],
  [/^\/admin\/isds-gateway(?:\/|$)/, '94_Odesilaci_brana_ISDS'],
  [/^\/isds-gateway\/callback(?:\/|$)/, '94_Odesilaci_brana_ISDS'],
  [/^\/admin\/electronic-signatures(?:\/|$)/, '95_Elektronicke_podpisy'],
  [/^\/admin\/tax-constants(?:\/|$)/, '96_Danove_konstanty'],
  [/^\/admin\/(?:users|roles|activity-log|cron-jobs)(?:\/|$)/, '97_Bezpecnost'],
  [/^\/admin\/update(?:\/|$)/, '98_Aktualizace'],
  [/^\/admin\/(?:diagnostics|support)(?:\/|$)/, '999_Reseni_problemu'],
  [/^\/profile\/mcp-server(?:\/|$)/, '101_MCP_server'],
  [/^\/profile\/api-tokens(?:\/|$)/, '99_API'],
  [/^\/activation(?:\/|$)|^\/hosting(?:\/|$)/, '100_Licence_a_aktivace'],
  [/^\/admin(?:\/|$)/, '92_Nastaveni'],
  [/^\/profile(?:\/|$)/, '97_Bezpecnost'],
  [/^\/portal(?:\/|$)/, '09_Klientsky_portal'],
  [/^\/crm(?:\/|$)/, '11_Zisk'],
  [/^\/stats(?:\/|$)/, '12_Trzby'],
  [/^\/purchase-stats(?:\/|$)/, '13_Naklady'],
  [/^\/$/, '10_Prehled'],
]

export function manualChapter(path: string): string | undefined {
  return MANUAL_CHAPTERS.find(([pattern]) => pattern.test(path))?.[1]
}
