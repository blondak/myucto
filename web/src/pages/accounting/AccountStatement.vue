...
function signedOpenAmount(it: OpenItemLine): number {
  return journalAmount({ amount: it.open_amount, is_red_storno: it.is_red_storno })
}
...
const selectedDelta = computed(() => {
  let cents = 0
  selected.value.forEach(it => { cents += Math.round(journalAmount(it) * 100) * (it.side === 'debit' ? 1 : -1) })
  return cents / 100
})
            ...
            <tbody class="divide-y divide-neutral-100">
              <tr v-for="(it, idx) in report.items" :key="`${it.entry_id}-${idx}`" class="hover:bg-neutral-50">
                ...
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="it.side === 'debit'">{{ formatMoney(journalAmount(it)) }}</template>
                </td>
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="it.side === 'credit'">{{ formatMoney(journalAmount(it)) }}</template>
                </td>
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-neutral-100">
              <tr v-for="it in openReport.items" :key="it.line_id"
                ...
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="it.side === 'debit' && it.open_amount !== 0">{{ formatMoney(signedOpenAmount(it)) }}</template>
                </td>
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="it.side === 'credit' && it.open_amount !== 0">{{ formatMoney(signedOpenAmount(it)) }}</template>
                </td>
          ...
          <table class="w-full text-sm">
            <tbody class="divide-y divide-neutral-100">
              <tr v-for="p in activePairing.items" :key="`${p.entry_id}-${p.line_no}`">
                ...
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="p.side === 'debit' && p.amount !== null">{{ formatMoney(journalAmount(p)) }}</template>
                </td>
                <td class="px-3 py-2 text-right font-mono whitespace-nowrap">
                  <template v-if="p.side === 'credit' && p.amount !== null">{{ formatMoney(journalAmount(p)) }}</template>
                </td>
                <td class="px-3 py-2 text-right whitespace-nowrap">
