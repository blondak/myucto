...
function errorMessage(error: any): string {
  ...
  return error?.response?.data?.error?.message || t('common.error')
}
...
async function prefill() {
  ...
  try {
    ...
    rows.value = draft.rows.map(row => ({ ...row }))
    toast.success(t('activation.prefill_done'))
  } catch (error: any) {
    toast.error(errorMessage(error))
  } finally { busy.value = false }
}
...
const actions = computed<ActionItem[]>(() => {
  ...
  if (currentStep.value === 2) return [
    { key: 'save', label: rows.value.length ? t('activation.save_and_continue') : t('activation.skip_opening'), icon: 'check', tier: 'primary', variant: 'primary', disabled: !openingBalanced.value, loading: busy.value, run: saveOpening },
...
<template>
  <div class="mx-auto max-w-6xl space-y-5">
    ...
    <ol class="grid grid-cols-2 gap-2 sm:grid-cols-5">
      <li v-for="step in steps" :key="step.number" class="rounded-lg border px-3 py-3 text-sm" :class="step.number === currentStep ? 'border-primary-500 bg-primary-50 text-primary-700' : step.number < currentStep ? 'border-success-500/40 bg-success-50 text-success-600' : 'border-neutral-200 text-neutral-400'"><span class="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full border text-xs font-semibold">{{ step.number < currentStep ? '✓' : step.number }}</span>{{ step.label }}</li>
    </ol>
    ...
    <section class="rounded-lg border border-neutral-200 bg-surface p-5 shadow-sm">
      ...
      <template v-else-if="currentStep === 2">
        ...
        <OpeningBalanceEditor v-model="rows" />
      </template>
      ...
      <template v-else-if="currentStep === 4">
        ...
        <label class="my-4 flex cursor-pointer items-start gap-3 rounded-lg border border-warning-500/30 bg-warning-50 p-4"><input v-model="executeConfirmed" type="checkbox" class="mt-1 rounded border-neutral-300 text-primary-600" /><span class="text-sm text-warning-700">{{ t('activation.execute_confirm', { n: status?.pending.total ?? job?.total_items ?? 0 }) }}</span></label>
