...
const sortedChecks = computed<DiagnosticCheck[]>(() =>
  [...(report.value?.checks ?? [])].sort((a, b) => (ORDER[a.status] ?? 9) - (ORDER[b.status] ?? 9)),
)
          ...
          >
            ...
            <dl class="mt-1.5 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-0.5 text-xs text-neutral-600">
              <div v-if="check.actual" class="flex gap-1.5">
                <dt class="text-neutral-500 shrink-0">{{ t('diagnostics.actual') }}:</dt>
                <dd class="font-mono break-all">{{ check.actual }}</dd>
              </div>
              <div v-if="check.expected" class="flex gap-1.5">
                <dt class="text-neutral-500 shrink-0">{{ t('diagnostics.expected') }}:</dt>
