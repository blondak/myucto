...
const COLUMN_PRESETS = [
  ...
  { key: 'accountant', labelKey: 'common.columns_preset_accountant', visibleKeys: [
    'number', 'client', 'type', 'issued', 'amount', 'status', 'base', 'vat_breakdown',
    'debit_accounts', 'credit_accounts', 'kh', 'locked',
  ] },
...
function mobileExtraFields(inv: InvoiceListItem): Array<{ key: string; label: string; value: string }> {
  const values: Record<string, string> = {
    ...
    amount_czk: inv.currency === 'CZK' ? formatMoney(inv.total_with_vat, 'CZK') : inv.exchange_rate ? formatMoney(inv.total_with_vat * inv.exchange_rate, 'CZK') : '—',
    base: formatMoney(inv.total_without_vat, inv.currency),
    vat: formatMoney(inv.total_vat, inv.currency),
    total: formatMoney(inv.total_with_vat, inv.currency),
    ...
    <div class="flex items-center justify-between mb-4 gap-3 flex-wrap">
      <div>
        ...
        <p class="text-sm text-neutral-500 mt-0.5">
          {{ t('invoice.subtitle_grouping') }}
          <template v-if="!loading && total > 0">
            <span class="ml-2 whitespace-nowrap">· {{ groupByMonth ? t('invoice.summary_count', { n: total, m: groups.length }) : t('invoice.summary_continuous', { n: total }) }}</span>
            <span v-if="total > loadedCount" class="ml-2 whitespace-nowrap">· {{ t('common.loaded_count', { loaded: loadedCount, total }) }}</span>
          </template>
        </p>
      </div>
