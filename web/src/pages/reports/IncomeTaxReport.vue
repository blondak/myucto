...
const warnings = computed(() => [...(state.value?.warnings ?? []), ...(exportWarning.value ? [exportWarning.value] : [])])
...
function checkStatusLabel(c: { ok: boolean; severity: string; na?: boolean }): string {
  ...
  return t('taxReturn.check_warning')
}
...
const regularSummary = computed(() => ({
  ...
  warning: regularChecks.value.filter((c) => !c.ok && !c.na).length,
}))
...
async function downloadXml() {
  ...
  try {
    const headers = await downloadApiFile(url)
    exportWarning.value = headers['x-validation-status'] === 'failed'
      ? t('taxReturn.xml_validation_warning')
      : Number(headers['x-business-errors'] ?? 0) > 0 ? t('taxReturn.xml_business_warning') : ''
    if ((type.value === 'po' || isFinal.value) && warnings.value.length === 0 && !prefinalize.value?.checks.some(c => !c.ok && !c.na)) {
...
const actions = computed<ActionItem[]>(() => {
  ...
  if (!isFinal.value) {
    ...
    a.push({ key: 'finalize', label: t('taxReturn.finalize'), icon: 'clipboardCheck', tier: 'secondary', variant: 'success',
      show: auth.canWrite('reports.finalize'), disabled: saving.value, run: finalize })
  } else {
...
<template>
  <div class="max-w-5xl">
    ...
    <div v-if="state && unsupportedCases.length" class="bg-surface border border-warning-500/40 rounded-lg p-4 mb-4">
      <div class="flex items-center justify-between mb-1 flex-wrap gap-2">
        <span class="text-sm font-semibold">{{ t('taxReturn.unsupported_title') }}</span>
        <span class="text-xs">
          <span v-if="unsupportedCases.length" class="text-warning-700 ml-2">{{ t('taxReturn.unsupported_warning_count', { n: unsupportedCases.length }) }}</span>
        </span>
      </div>
      <p class="text-xs text-neutral-500 mb-3">
        {{ t('taxReturn.unsupported_warning_hint') }}
      </p>
      <ul class="space-y-2">
        <li v-for="c in unsupportedCases" :key="c.key"
    ...
    <div v-if="state && prefinalize" class="bg-surface border rounded-lg p-4 mb-4"
      :class="regularSummary.warning > 0 ? 'border-warning-500/40' : 'border-neutral-200'">
      ...
      <ul class="space-y-2">
        <li v-for="c in regularChecks" :key="c.key" class="border rounded-md p-2.5 text-sm" :class="checkTone(c)">
          ...
          <div class="text-xs mt-1 opacity-90">
            ...
            <template v-else-if="c.value?.message">{{ c.value.message }}</template>
          </div>
        </li>
      </ul>
    </div>
        ...
        <fieldset :disabled="isFinal" class="space-y-4">
            ...
            <div class="bg-surface border border-neutral-200 rounded-lg p-4 space-y-3">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <label class="text-sm">{{ t('taxReturn.filing_deadline') }}
                  <DateInput v-model="inputs.filing_deadline" class="mt-1 w-full h-9 px-2 border border-neutral-300 rounded-md" />
          ...
          <template v-else>
            ...
            <div class="bg-surface border border-neutral-200 rounded-lg p-4 space-y-3">
              ...
              <div v-for="(child, childIndex) in profile.children" :key="childIndex" class="border border-neutral-200 rounded-md p-3 space-y-2">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-2"><input v-model="child.first_name" :placeholder="t('taxReturn.first_name')" class="h-9 px-2 border border-neutral-300 rounded-md text-sm" /><input v-model="child.last_name" :placeholder="t('taxReturn.last_name')" class="h-9 px-2 border border-neutral-300 rounded-md text-sm" /><input v-model="child.birth_number" :placeholder="t('taxReturn.birth_number')" class="h-9 px-2 border border-neutral-300 rounded-md text-sm" /><DateInput v-model="child.birth_date" class="h-9 px-2 border border-neutral-300 rounded-md text-sm" /><button type="button" @click="profile.children!.splice(childIndex, 1)" class="h-9 px-3 rounded-md border border-danger-500 text-danger-600 text-sm"><span aria-hidden="true">×</span> {{ t('common.delete') }}</button></div>
          ...
          <div v-if="variant === 'dodatecne'" class="bg-surface border border-neutral-200 rounded-lg p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <label class="text-sm">{{ t('taxReturn.d_zjist') }}
              <DateInput v-model="inputs.d_zjist" class="mt-1 w-full h-9 px-2 border border-neutral-300 rounded-md" /></label>
        ...
        <div v-if="type === 'po'" class="bg-surface border border-neutral-200 rounded-lg p-5">
          ...
          <div v-if="editingOverrideId !== null" class="mb-3 rounded-md border border-neutral-200 bg-neutral-50 p-3 grid grid-cols-2 md:grid-cols-6 gap-2 items-end">
            <label class="text-xs">{{ t('taxReturn.overrides_from') }}
              <DateInput v-model="overrideForm.effective_from" class="mt-1 w-full h-8 px-2 border border-neutral-300 rounded-md text-sm" /></label>
            <label class="text-xs">{{ t('taxReturn.overrides_to') }}
              <DateInput v-model="overrideForm.effective_to" :placeholder="t('taxReturn.overrides_to_open')"
